...
export class Bridge implements BridgeInterface {
  ...
  constructor(minterCanister: string, Ic: IcConnector) {
  ...
  public async get_wrapped_token_address(
    ...
    const wrappedToken = await contract.methods
  ...
  async mintOrder(
    ...
    const result = await contract.methods.mint(encodedOrderBytes).send();
